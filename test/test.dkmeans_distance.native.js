...
var dkmeansDistance = tryRequire( resolve( __dirname, './../lib/dkmeans_distance.native.js' ) );
var opts = {
...
tape( 'the function has an arity of 6', opts, function test( t ) {
	t.strictEqual( dkmeansDistance.length, 6, 'returns expected value' );
	t.end();
});
...
tape( 'the function calculates the distance according to a specified distance metric', opts, function test( t ) {
	...
	z = dkmeansDistance( x.length, 'sqeuclidean', x, 1, y, 1 );
...
tape( 'if provided an `N` parameter less than or equal to `0`, the function returns `NaN`', opts, function test( t ) {
	...
	z = dkmeansDistance( 0, 'sqeuclidean', x, 1, y, 1 );
	t.strictEqual( isAlmostSameValue( z, NaN, 0 ), true, 'returns expected value' );

	z = dkmeansDistance( -1, 'sqeuclidean', x, 1, y, 1 );
	t.strictEqual( isAlmostSameValue( z, NaN, 0 ), true, 'returns expected value' );
...
tape( 'the function supports stride parameters', opts, function test( t ) {
	...
	z = dkmeansDistance( 4, 'sqeuclidean', x, 2, y, 2 );
...
tape( 'the function supports view offsets', opts, function test( t ) {
	...
	z = dkmeansDistance( 4, 'sqeuclidean', x1, 2, y1, 2 );
...
tape( 'the function supports all documented distance metrics', opts, function test( t ) {
	...
	for ( i = 0; i < values.length; i++ ) {
		out = dkmeansDistance( 2, values[ i ], x, 1, y, 1 );
