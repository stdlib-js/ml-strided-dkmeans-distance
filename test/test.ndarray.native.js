...
var dkmeansDistance = tryRequire( resolve( __dirname, './../lib/ndarray.native.js' ) );
var opts = {
	'skip': ( dkmeansDistance instanceof Error )
};
...
tape( 'main export is a function', opts, function test( t ) {
	...
	t.strictEqual( typeof dkmeansDistance, 'function', 'main export is a function' );
	t.end();
});

tape( 'the function has an arity of 8', opts, function test( t ) {
	t.strictEqual( dkmeansDistance.length, 8, 'returns expected value' );
	t.end();
});

tape( 'the function calculates the distance according to a specified distance metric', opts, function test( t ) {
	var x;
	var y;
	var z;
	...
	z = dkmeansDistance( x.length, 'sqeuclidean', x, 1, 0, y, 1, 0 );
...
tape( 'if provided an `N` parameter less than or equal to `0`, the function returns `NaN`', opts, function test( t ) {
	...
	z = dkmeansDistance( 0, 'sqeuclidean', x, 1, 0, y, 1, 0 );
	t.strictEqual( isAlmostSameValue( z, NaN, 0 ), true, 'returns expected value' );

	z = dkmeansDistance( -1, 'sqeuclidean', x, 1, 0, y, 1, 0 );
	t.strictEqual( isAlmostSameValue( z, NaN, 0 ), true, 'returns expected value' );
...
tape( 'the function supports stride parameters', opts, function test( t ) {
	...
	z = dkmeansDistance( 4, 'sqeuclidean', x, 2, 0, y, 2, 0 );
...
tape( 'the function supports negative stride parameters', opts, function test( t ) {
	...
	z = dkmeansDistance( 4, 'sqeuclidean', x, -2, x.length-2, y, -2, y.length-2 );
...
tape( 'the function supports offset parameters', opts, function test( t ) {
	...
	z = dkmeansDistance( 4, 'sqeuclidean', x, 2, 1, y, 2, 1 );
...
tape( 'the function supports all documented distance metrics', opts, function test( t ) {
	...
	y = new Float64Array( [ 1.0, 3.0, 5.0, 5.0 ] );

	for ( i = 0; i < values.length; i++ ) {
		out = dkmeansDistance( 2, values[ i ], x, 1, 0, y, 1, 0 );
		t.strictEqual( typeof out, 'number', 'returns expected value for metric: ' + values[ i ] );
	}
	t.end();
});
